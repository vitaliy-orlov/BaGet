...
using BaGet.Core.Protocol.Internal.Extensions;
...
namespace BaGet.Core.Protocol.Internal
{
    ...
    internal class IndexingService : IIndexingService
    {
        ...
        public async Task<IndexingResultType> IndexAsync(Stream packageStream, CancellationToken cancellationToken)
        {
            ...
            if (result == PackageAddResultType.PackageAlreadyExists)
            {
                _logger.LogWarning(
            ...
            if (result != PackageAddResultType.Success)
            {
                _logger.LogError($"Unknown {nameof(PackageAddResultType)} value: {{PackageAddResult}}", result);

                throw new InvalidOperationException($"Unknown {nameof(PackageAddResultType)} value: {result}");
            }
            ...
            return IndexingResultType.Success;
        }
